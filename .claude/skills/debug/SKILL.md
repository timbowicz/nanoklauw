...
### 0. Duplicate Processes / Double Messages

**Symptoms:** Messages sent twice, WhatsApp `Stream Errored (conflict)` in logs.

**Root cause:** Two NanoClaw processes running simultaneously, both connecting to WhatsApp.

**Check:**
```bash
# Should show exactly ONE node process
ps aux | grep 'node.*index' | grep -v grep

# Verify only the system-level service is active
systemctl status nanoklauw        # Should be active
systemctl --user status nanoclaw  # Should be inactive/not found
```

**Fix:** This server uses the system-level `nanoklauw.service`. A user-level `nanoclaw.service` existed previously and has been disabled. If it somehow gets re-enabled:
```bash
systemctl --user stop nanoclaw && systemctl --user disable nanoclaw
```
...
### 0b. Container Permission Errors (EACCES)

**Symptoms:** `EACCES: permission denied, mkdir '/home/node/.claude/debug'` or `EACCES: permission denied, unlink '/workspace/ipc/input/...'`

**Root cause:** Host runs as root (uid 0), containers run as `node` (uid 1000). Mounted directories must be writable by uid 1000.

**Fix:** The code handles this automatically via `chown` in `container-runner.ts`, but if permissions get reset:
```bash
