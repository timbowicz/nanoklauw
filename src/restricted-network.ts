...
export async function ensureRestrictedNetwork(): Promise<void> {
  ...
  try {
    ...
    logger.info(
      { network: RESTRICTED_NETWORK_NAME, subnet: RESTRICTED_NETWORK_SUBNET },
...
export async function refreshAllowedIps(): Promise<void> {
  ...
  const allDomains = [
    ...new Set([...RESTRICTED_ALLOWED_DOMAINS, ...dynamicDomains]),
  ];
  ...
  await Promise.all(
    allDomains.map(async (domain) => {
      try {
        ...
        logger.warn(
          { domain, err },
          'Failed to resolve domain for restricted network',
        );
      }
    }),
  );
  ...
  try {
    await execFileAsync('iptables', ['-F', tmpChain]);
  } catch {}
  try {
    await execFileAsync('iptables', ['-X', tmpChain]);
  } catch {}
  try {
...
async function iptablesAppendTo(
  chain: string,
  ruleArgs: string[],
): Promise<void> {
  try {
    ...
    logger.error(
      { err, chain, rule: ruleArgs.join(' ') },
      'Failed to append iptables rule',
    );
  }
}
